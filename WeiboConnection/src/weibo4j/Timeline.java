...
public class Timeline extends Weibo {
	...
	public StatusWapper getMentions(Long since_id) throws WeiboException {
		return Status.constructWapperStatus(client.get(
	 ...
	 * @version weibo4j-V2 1.0.0
	 * @see <a
	 *      href="http://open.weibo.com/wiki/2/direct_messages/new.json">direct_messages/new</a>
	 * @since JDK 1.5
	 */
	public DirectMessage sendDirectMessage(String uid, String text)
			throws WeiboException {
		return new DirectMessage(client.post(WeiboConfig.getValue("baseURL")
	 ...
	 * @author Felix
	 */
	public JSONObject getUnreadCount(String uid) throws WeiboException {
		return client
				.get(WeiboConfig.getValue("base_rmURL")
						+ "remind/unread_count.json",
						new PostParameter[] { 
						new PostParameter("uid", uid),
						new PostParameter("invitation_platform",1)})
				.asJSONObject();
	}
	 ...
	 * 				发送文本
	 * @param 
	 * 				imgid 图片id，为图片第一次上传后获得的vfid
	 * @return
	 * @throws WeiboException
	 * @author Felix
	 */
	public DirectMessage sendDirectMessageInter(String toUID, String text,
			String imgid) throws WeiboException {
		String fids = "";
		if (imgid.length() > 1) {
			JSONObject resRepost = repostTovfid(toUID, imgid);
			if (resRepost.has("tovfid")) {
				fids += imgid + "," + resRepost.optString("tovfid");
			}
		}

		return new DirectMessage(client.post(WeiboConfig.getValue("interURL")
				+ "direct_messages/new.json", new PostParameter[] {
				new PostParameter("uid", toUID),
				new PostParameter("text", text),
				new PostParameter("fids", fids) }));
	}
	 ...
	 * @author Felix
	 */
	public JSONObject repostTovfid(String toUID, String imgid)
			throws WeiboException {
		return client.get(
				WeiboConfig.getValue("uploadURL") + "/mss/repost.json",
				new PostParameter[] { new PostParameter("fid", imgid),
						new PostParameter("tuid", toUID) }).asJSONObject();
	}
